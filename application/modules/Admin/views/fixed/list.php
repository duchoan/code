    ...
    <ul>
        ...
        <li class="option-item item-list-status"><a
            ...
            <ul class="ul-list cookie-name" cookie_name="content_list_price" catid="allsearch_text"
                ...
                <li><span class="tick-active">Tên tuyến</span></li>
                <li><span class="tick-active">Thành phố</span></li>
            ...
            <?php foreach ($object_item as $obj) { ?>
                <tr>
                    ...
                    <td style="min-width: 80px">
                        ...
                        <?php if(Check_per_action($table,$iduser,3)){;?>
                            <?php if($obj->show==1){?>
                                <span title="Gỡ bài" data-id="<?php echo $obj->id;?>" class="publish-item publish-on"></span>
                            <?php }else{?>
                                <span title="Đăng bài" data-id="<?php echo $obj->id;?>" class="publish-item publish-off"></span>
                            <?php }?>
                        <?php }?>
                    ...
                    <td>
                        <a href="<?php if(Check_per_action($table,$iduser,2)){;?><?php echo base_url(); ?>admin-direct/edit?table=<?php echo $this->input->get('table'); ?>&id=<?php echo $obj->id; ?><?php }else{echo 'javascript:void(0);'; }?>"
                           class="list-name"><?php echo $this->Admin_model->get_object('district',array('id'=>$obj->district_1),'title').' - '.$this->Admin_model->get_object('district',array('id'=>$obj->district_2),'title'); ?></a>
                    </td>
                    <td>
                        <?php echo $this->Admin_model->get_object('city',array('id'=>$obj->city),'title'); ?>
                    </td>
                </tr>
                <!-- end row item-->
            <?php } ?>
                    ...
                    <label>
                        <select class="action_selecte" name="perpage">
                            ...
                            for ($i = 10; $i < 200; $i) {
                                ...
                                }elseif($i==20){echo 'selected';}  ?> value="<?php echo $i; ?>"><?php echo $i; ?></option>
                                <?php
                                if ($i < 50) {
                                    $i = $i + 10;
                                } else {
